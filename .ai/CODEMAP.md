...
- `skills/handling-assigned-desk-work/SKILL.md` — passively handle only owner-named task ids, hydrate their context, and post one compact blocker or ready-for-review outcome while leaving queue and lifecycle control with the owner. Its `rundesk.json` declares the scoped API key.
