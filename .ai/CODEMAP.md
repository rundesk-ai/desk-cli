...
- `skills/handling-assigned-desk-work/SKILL.md` — passively handle only owner-requested tasks, hydrate their context, mention the Desk owner in one compact terminal outcome, and leave queue and lifecycle control with the owner. Its reference holds the optional AGENTS/CLAUDE adoption template; `rundesk.json` declares the scoped API key.
- `skills/managing-your-desk/SKILL.md` — choose an exact injected profile, probe desk versus human mode, run the Desk-owned weekly commitment top-down, hydrate task/page/asset context, and guard writes. `rundesk.json` declares the API key; references hold conditional queue adoption/task-brief examples, identity, task, project/page/asset, and owner desk-management detail. Its active queue authority intentionally conflicts with `handling-assigned-desk-work`; grant only one workflow to an agent.
