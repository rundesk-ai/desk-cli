...
- `src/desk_cli/cli.py` (`main`) — the real `desk` surface: wraps the full-API parser, adds profiles/update/uninstall/help, reshapes the desk group into top-level `whoami`/`inbox`, resolves credentials, dispatches.
...
| `cli.py` | Installable command surface: profile store commands, update/uninstall/help, `whoami`/`inbox`, credential resolution + dispatch, passive update notice. |
...
- `RundeskClient` — bearer client; 60+ public methods (test gate asserts ≥60) covering account/changelog, desk agent surface, jobs, projects, pages (+search), tasks (+deadline/recurring/move/comments), weeks, assets (get/search/list/upload/rename/delete), owner desk management. Constructed with explicit `base_url`/`api_key` by `cli.py`, or resolves from env/`.env` when built bare.
- `Paths` — every REST path relative to `/api/v1`, the single place paths are defined (48 members). Nothing else hardcodes a path.
...
- Top-level groups: `whoami`, `account`, `changelog`, `desk` (show/inbox/memory/memory-set/memory-grep/memory-patch), `jobs`, `projects`, `page`, `tasks`, `week`, `weeks`, `asset`, `desks`. Built in `build_parser`; each `cmd_*` handler renders.
- `_emit` — prints text as-is, or pretty JSON for non-str payloads / `--json`.
- `_CONFIRM_GATED` — the 9 `(command, action)` pairs that hard-delete and require `--confirm` (projects/page/tasks/jobs/desks delete, tasks comment-delete, asset delete-task/-project/-page).
- `cli.py` reshapes this tree: drops the nested `desk` group, repoints `whoami` at `GET /desk` (desk identity), promotes `desk inbox` to top-level `inbox`, and hides `--env-file`.
...
- `test_cli.py` — auto-discovers every leaf command and walks it through `cli.main()` against a monkeypatched `urllib.request.urlopen`; asserts exit 0 + credentialed request. Gate: every public client method is reachable from the tree (no orphans).
- `test_rundesk.py` — the REST client's request/response suite (131 tests).
